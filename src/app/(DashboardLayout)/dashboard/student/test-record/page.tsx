...
export default function TestRecordsPage() {
  ...
  const barData = useMemo(() => {
    ...
    return [
      {
        name: "Total Score",
        value: data.reduce((a, b) => a + b.score, 0),
      },
      {
        name: "Average Score",
        value: data.reduce((a, b) => a + b.score, 0) / (data.length || 1),
      },
    ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard title="Performance Over Tests">
          <div className="flex justify-between mb-2">
            ...
            <div className="font-semibold">{months[monthIndex1] || "-"}</div>
            <button
        ...
        <ChartCard title="Monthly Analysis">
          <div className="flex justify-between mb-2">
            ...
            <div className="font-semibold">{months[monthIndex2] || "-"}</div>
            <button
        ...
        <table className="min-w-full border-collapse table-auto">
          <thead className="bg-gray-100 sticky top-0 z-10">
            <tr>
              ...
              <th className="p-3 border-b border-gray-300 text-left">Course</th>
              <th className="p-3 border-b border-gray-300 text-left">Subject</th>
              <th className="p-3 border-b border-gray-300 text-left">Chapter</th>
              <th className="p-3 border-b border-gray-300 text-left">Score</th>
              <th className="p-3 border-b border-gray-300 text-left">%</th>
              <th className="p-3 border-b border-gray-300 text-left">Correct</th>
              <th className="p-3 border-b border-gray-300 text-left">Incorrect</th>
              <th className="p-3 border-b border-gray-300 text-left">Unanswered</th>
            </tr>
          </thead>
          <tbody>
            {filteredRecords.map((r, i) => (
              <tr key={r._id} className={`${i % 2 === 0 ? "bg-gray-50" : ""} hover:bg-gray-100 transition`}>
                <td className="p-2 border-b">{new Date(r.date).toLocaleDateString()}</td>
                <td className="p-2 border-b">{r.course}</td>
                <td className="p-2 border-b">{r.subject}</td>
                <td className="p-2 border-b">{r.chapter}</td>
                <td className="p-2 border-b">{r.score}</td>
                <td className="p-2 border-b">{r.percentage}%</td>
                <td className="p-2 border-b">{r.correct}</td>
                <td className="p-2 border-b">{r.incorrect}</td>
                <td className="p-2 border-b">{r.unanswered.length}</td>
              </tr>
            ))}
          </tbody>
        </table>
